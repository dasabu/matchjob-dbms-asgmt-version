...
export default function UserUpsertModal({
    ...
    try {
      ...
      if (!userData) {
        ...
        if (response && response.data) {
          ...
          toast({
            title: 'Có lỗi xảy ra trong quá trình tạo người dùng',
            // description: response.data.message,
            variant: 'destructive',
          })
        }
      ...
      toast({
        title: 'Có lỗi xảy ra trong quá trình cập nhật thông tin người dùng',
  ...
  const { data: fetchedRoles } = useQuery({
    queryKey: ['roles'],
    queryFn: () => getRolesApi('current=1&pageSize=100'),
  })
  ...
  const { data: fetchedCompanies } = useQuery({
    queryKey: ['companies'],
    queryFn: () => getCompaniesApi('current=1&pageSize=100'),
  })
